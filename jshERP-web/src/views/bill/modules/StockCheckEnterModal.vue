<template>
    <j-modal :title="title" :width="width" :visible="visible" :confirmLoading="confirmLoading" :keyboard="false"
                ...
                <a-row class="form-row" :gutter="24">
                    ...
                    <a-col :lg="6" :md="12" :sm="24">
                        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="批量导入" data-step="3"
                            data-title="批量导入">
                            <a-button type="primary" @click="onImportStockCheck(prefixNo)"><a-icon
                                    type="import" />导入盘点实存信息</a-button>
                        </a-form-item>
                    </a-col>
        ...
        <import-stock-check-modal ref="importStockCheckModalForm"
            @ok="importStockCheckModalFormOk"></import-stock-check-modal>
    </j-modal>
</template>
<script>
import ImportStockCheckModal from '../dialog/ImportStockCheckModal.vue'
    ...
    data() {
        return {
            ...
            url: {
                ...
                importExcelUrl: "/depotItem/importStockCheckExcel",
            }
        }
    },
    created() {
    ...
    methods: {
        ...
        editAfter() {
            ...
            if (this.action === 'add') {
                ...
                this.$nextTick(() => {
                    this.form.setFieldsValue(pick(this.model, 'operTime', 'number', 'remark', 'changeAmount'))
                });
                // 加载子表数据
                let params = {
