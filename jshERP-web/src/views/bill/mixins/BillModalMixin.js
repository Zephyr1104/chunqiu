...
export const BillModalMixin = {
  ...
  methods: {
      ...
      getAction(url, params).then(res => {
        if (res && res.code === 200) {
          ...
          for (let i = 0; i < tab.dataSource.length; i++) {
            ...
            info.currentStock = (info.stock + info.operNumber) -0
        ...
        if (res.code === 200) {
          ...
          for (let item of that.materialTable.columns) {
            if (item.key == 'mealId') {
              ...
              for (let i = 0; i < arr.length; i++) {
                let mealInfo = {};
                mealInfo.value = arr[i].id + '' //注意-此处value必须为字符串格式
                mealInfo.text = arr[i].name
                mealInfo.title = arr[i].name
                item.options.push(mealInfo)
              }
            }
          }
        }
    ...
    handleBatchSetMeal() {
      this.$refs.batchSetMealModalForm.add();
      this.$refs.batchSetMealModalForm.title = "批量切换餐次";
      this.$refs.batchSetMealModalForm.disableSubmit = false;
    },
    handleBatchSetAnotherDepot() {
      this.$refs.batchSetAnotherDepotModalForm.add();
      this.$refs.batchSetAnotherDepotModalForm.title = "批量切换仓库";
      this.$refs.batchSetAnotherDepotModalForm.disableSubmit = false;
    },
            ...
            if (mList && mList.length) {
              for (let i = 0; i < detailArr.length; i++) {
                ...
                if (item.currentStock) {
                  ...
                  item.operNumber = (currentStock - stock) - 0
                  item.allPrice = (unitPrice * item.operNumber).toFixed(2) - 0
                }
    ...
    batchSetMealModalFormOk(mealId) {
      this.getAllTable().then(tables => {
        return getListData(this.form, tables)
      }).then(allValues => {
        //获取单据明细列表信息
    ...
    batchSetAnotherDepotModalFormOk(depotId) {
      this.getAllTable().then(tables => {
        return getListData(this.form, tables)
      }).then(allValues => {
        //获取单据明细列表信息
    ...
    onValueChange(event) {
      ...
      let param, snList, batchNumber, operNumber, unitPrice, allPrice, taxRate, taxMoney, taxLastMoney, currentStock
      switch (column.key) {
        case "depotId":
        ...
        case "batchNumber":
          ...
          break;
        case "currentStock":
          currentStock = value - 0  //实存
          taxRate = row.taxRate - 0 //税率
          unitPrice = row.unitPrice - 0 //单价
          // 数量
          operNumber = (currentStock - row.stock) - 0
          allPrice = (unitPrice * operNumber).toFixed(2) - 0
          taxMoney = ((taxRate * 0.01) * allPrice).toFixed(2) - 0
          taxLastMoney = (allPrice + taxMoney).toFixed(2) - 0
          target.setValues([{ rowKey: row.id, values: { allPrice: allPrice, operNumber: operNumber, taxMoney: taxMoney, taxLastMoney: taxLastMoney } }])
          target.recalcAllStatisticsColumns()
          that.autoChangePrice(target)
          break;
        case "operNumber":
    ...
    // 导入盘点录入
    onImportStockCheck(prefixNo) {
      this.$refs.importStockCheckModalForm.add(prefixNo);
    },
      // 导入采购协议价录入
      onImportAgreementPrice(prefixNo) {
        this.$refs.importAgreementPriceModalForm.add(prefixNo);
      },
       // 导入营养成分录入
       onImportNutritional(prefixNo) {
        this.$refs.importNutritionalModalForm.add(prefixNo);
      },
    importStockCheckModalFormOk(data) {
      this.materialTable.dataSource = data
      // this.$nextTick(() => {
      //   let discountLastMoney = 0
      //   for (let i = 0; i < data.length; i++) {
    ...
    // 调拨出库导入
    onImportTransfer(prefixNo) {
      this.$refs.importTransferModal.add(prefixNo);
    },
    importTransferModalOk(data) {
      this.materialTable.dataSource = data
    },
