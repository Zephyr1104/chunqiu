...
import {findBillDetailByNumber,findMaterialRByNumber,findBySelectSup, findBySelectCus, findBySelectRetail, getUserList, getAccount, waitBillCount,
  ...
  methods: {
    ...
    myHandleREdit(record) {
      if(record.status === '0') {
        ...
        //查询单条报告信息
        findMaterialRByNumber({ reportId: record.id }).then((res) => {
          if (res && res.code === 200) {
            let item = res.data
            console.log(item);
        ...
        this.$message.warning("抱歉，只有未审核的报告才能编辑，请先进行反审核！")
      }
    },
