    ...
    <a-form :form="form">
              ...
              <div :style="tableWidthRetail">
                <a-table ref="table" size="middle" bordered rowKey="id" :pagination="false" :loading="loading"
                  ...
                  <template slot="customBarCode" slot-scope="text, record">
                    <div :style="record.imgName ? 'float:left;line-height:30px' : 'float:left;'">{{ record.barCode }}
                    </div>
                    <a-popover placement="right" trigger="click">
                      ...
                      <div class="item-info" v-if="record.imgName"><img v-if="record.imgName"
                          :src="getImgUrl(record.imgName, record.imgSmall)" class="item-img" title="查看大图" /></div>
                    </a-popover>
                  </template>
                </a-table>
              </div>
      ...
      <template v-else-if="billType === '采购订单'">
        <section ref="print" id="purchaseOrderPrint">
          ...
          <div style="width:210mm;text-align: center;font-size: 25px;">广东厨誉餐饮管理有限公司采购订单</div>
          ...
          <div style="width:210mm;display:flex;justify-content: space-between;">
            ...
            <div style="display:flex;flex-direction:column;">
              ...
              <span>关联订单:<a @click="myHandleDetail(model.linkNumber)">{{ model.linkNumber }}</a></span>
            </div>
          </div>
          ...
          <div style="width:210mm;display:flex;justify-content: space-between;">
            <div style="display:flex;flex-direction: column;">
              <span>制单人：{{ model.creatorName }}</span><br />
              <span>财务审核:____________</span>
            </div>
            <div style="display:flex;flex-direction:column;">
              <span>结算账户：{{ model.accountName }}</span><br />
              <span>支付订金：{{ model.changeAmount }}</span>
            </div>
            <div style="display:flex;flex-direction:column;">
              <span>审核人：{{ model.auditorName }}</span><br />
              <!-- <span>扣除订金：{{ model.deposit }}</span> -->
              <span>供应商签名:____________</span>
            </div>
          </div>
      ...
      <template v-else-if="billType === '采购入库'">
        <section ref="print" id="purchaseInPrint">
          ...
          <div style="width:210mm;text-align: center;font-size: 25px;">广东厨誉餐饮管理有限公司采购入库单</div>
          ...
          <div style="width:210mm;display:flex;justify-content: space-between;">
            <div style="display:flex;flex-direction: column;">
              <span>制单人：{{ model.creatorName }}</span>
            ...
            <div style="display:flex;flex-direction:column;">
              <span>审核人：{{ model.auditorName }}</span>
              <!-- <span>扣除订金：{{ model.deposit }}</span> -->
      ...
      <template v-else-if="billType === '采购退货出库'">
        <section ref="print" id="purchaseBackPrint">
          ...
          <div style="width:210mm;text-align: center;font-size: 25px;">广东厨誉餐饮管理有限公司采购退货出库单</div>
      ...
      <template v-else-if="billType === '销售出库'">
        <section ref="print" id="saleOutPrint">
          ...
          <div style="width:210mm;text-align: center;font-size: 25px;">广东厨誉餐饮管理有限公司销售出库单</div>

          <div style="width:210mm;display:flex;justify-content: space-between;">
            <div style="display:flex;flex-direction:column;">
              <span>客户:{{ model.organName }}</span>
              <span>单据编号:{{ model.number }}</span>
            </div>
            <div style="display:flex;flex-direction:column;">
              <span>单据日期:{{ model.operTimeStr }}</span>
          ...
          <div style="width:210mm;display:flex;justify-content: space-between;">
            <div style="display:flex;flex-direction: column;">
              <span>结算账户：{{ model.accountName }}</span>
              <span>销售人员：{{ model.salesManStr }}</span>
            </div>
            <div style="display:flex;flex-direction:column;">
              <span>本次收款：{{ model.changeAmount }}</span>
              <span>其它费用：{{ model.otherMoney }}</span>
            </div>
            <div style="display:flex;flex-direction:column;">
              <span>制单人：{{ model.creatorName }}</span>
      ...
      <template v-else-if="billType === '其它入库'">
        <section ref="print" id="otherInPrint">
          ...
          <div style="width:210mm;text-align: center;font-size: 25px;">广东厨誉餐饮管理有限公司其它入库单</div>
          ...
          <div style="width:210mm;display:flex;justify-content: space-between;">
            ...
            <div style="display:flex;flex-direction:column;">
              <span>单据日期:{{ model.operTimeStr }}</span>
              <!-- <span>关联单据:<a @click="myHandleDetail(model.linkNumber)">{{ model.linkNumber }}</a></span> -->
            </div>
          </div>
      ...
      <template v-else-if="billType === '其它出库'">
        <section ref="print" id="otherOutPrint">
          ...
          <div style="width:210mm;text-align: center;font-size: 25px;">广东厨誉餐饮管理有限公司领料出库单</div>
          ...
          <div style="width:210mm;padding-top:10px;display:flex;justify-content: space-between;font-size: 19px;">
            <div style="display:flex;flex-direction:column;">
              <span>制单人：{{ model.creatorName }}</span>
              <span>审核人：{{ model.auditorName }}</span><br />
              <span>白联(仓库):</span>
            ...
            <div style="display:flex;flex-direction:column;">
              <span>仓管员:__________</span><br /><br />
              <span>红联(学校):</span>
            </div>
            <div style="display:flex;flex-direction:column;">
              <span>领料人:__________</span><br /><br />
              <span>黄联(存根):</span>
            </div>
          </div>
      ...
      <template v-else-if="billType === '调拨出库'">
        <section ref="print" id="allocationOutPrint">
          ...
          <div style="width:210mm;text-align: center;font-size: 25px;">广东厨誉餐饮管理有限公司调拨出库单</div>

          <div style="width:210mm;display: flex;justify-content: space-between;font-size: 19px;">
            <div style="display: flex;flex-direction: column;">
              <span :span="6">单据日期:{{ model.operTimeStr }}</span>
            </div>
            <div style="display: flex;flex-direction: column;"></div>
            <div style="display: flex;flex-direction: column;">
              <span :span="6">单据编号:{{ model.number }}</span>
            </div>
          </div>
          ...
          <div style="width:210mm;padding-top:10px;display:flex;justify-content: space-between;font-size: 19px;">
            ...
            <div style="display:flex;flex-direction:column;">
              <span>送货人:__________</span><br /><br />
              <span>红联(学校):</span>
            </div>
            <div style="display:flex;flex-direction:column;">
              <span>签收人:__________</span><br /><br />
              <span>黄联(存根):</span>
            </div>
          </div>
      ...
      <template v-else-if="billType === '盘点录入'">
        <section ref="print" id="stockCheckEnterPrint">
          ...
          <div :style="tableWidth">
            <a-table ref="table" size="large" bordered rowKey="id" :pagination="false" :columns="columns"
...
export default {
  ...
  data() {
    return {
      ...
      defColumns: [],
      retailOutColumns: [
        { title: '仓库名称', dataIndex: 'depotName', width: "120px" },
        { title: '条码', dataIndex: 'barCode' },
        { title: '名称', dataIndex: 'name' },
        { title: '规格', dataIndex: 'standard', width: "120px" },
      ...
      purchaseOrderColumns: [
        ...
        { title: '多属性', dataIndex: 'sku' },
        { title: '数量', dataIndex: 'operNumber' },
        { title: '已入库', dataIndex: 'finishNumber', width: "25%" },
        { title: '单价', dataIndex: 'unitPrice' },
        { title: '金额', dataIndex: 'allPrice' },
      ...
      purchaseBackColumns: [
        ...
        // { title: '多属性', dataIndex: 'sku' },
        { title: '数量', dataIndex: 'operNumber' },
        { title: '已出库', dataIndex: 'finishNumber', width: "25%" },
        { title: '单价', dataIndex: 'unitPrice' },
        { title: '金额', dataIndex: 'allPrice' },
        // { title: '税率(%)', dataIndex: 'taxRate' },
        // { title: '税额', dataIndex: 'taxMoney' },
      ...
      otherInColumns: [
        ...
        { title: '重量', dataIndex: 'weight' },
        { title: '仓位货架', dataIndex: 'position' },
        { title: '备注', dataIndex: 'remark',width: "80px" }
      ],
      otherOutColumns: [
        { title: '仓库名称', dataIndex: 'depotName', width: "120px" },
        { title: '条码', dataIndex: 'barCode' },
        { title: '名称', dataIndex: 'name', width: "150px" },
        { title: '规格', dataIndex: 'standard', width: "150px" },
        // { title: '型号', dataIndex: 'model' },
        // { title: '颜色', dataIndex: 'color' },
        // { title: '扩展信息', dataIndex: 'materialOther' },
        // { title: '库存', dataIndex: 'stock' },
      ...
      allocationOutColumns: [
        { title: '仓库名称', dataIndex: 'depotName', width: "120px" },
        { title: '条码', dataIndex: 'barCode',width:"60px" },
        { title: '名称', dataIndex: 'name', width: "120px" },
        { title: '餐次', dataIndex: 'mealName',width: "80px" },
        { title: '规格', dataIndex: 'standard', width: "150px" },
        // { title: '型号', dataIndex: 'model' },
        // { title: '颜色', dataIndex: 'color' },
        // { title: '扩展信息', dataIndex: 'materialOther' },
        // { title: '库存', dataIndex: 'stock' },
        { title: '调入仓库', dataIndex: 'anotherDepotName', width: "120px" },
        { title: '单位', dataIndex: 'unit' },
        // { title: '多属性', dataIndex: 'sku' },
        { title: '数量', dataIndex: 'operNumber' },
        { title: '单价', dataIndex: 'unitPrice' },
  ...
  methods: {
    ...
    allocationOutExportExcel() {
      let list = []
      let head = '仓库名称,条码,名称,规格,型号,颜色,餐次,库存,调入仓库,单位,多属性,数量,单价,金额,备注'
      for (let i = 0; i < this.dataSource.length; i++) {
        let item = []
        let ds = this.dataSource[i]
        item.push(ds.depotName, ds.barCode, ds.name, ds.standard, ds.model, ds.color, ds.mealName, ds.stock, ds.anotherDepotName, ds.unit,
    ...
    Print() {
      ...
      // <div style="width:148mm;text-align: center;font-size: 25px;" v-else>${this.billType}</div>
