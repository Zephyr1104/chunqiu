<template>
  <div ref="container">
    <a-modal :title="title" :width="1250" :visible="visible" :getContainer="() => $refs.container"
      ...
      <div class="table-page-search-wrapper" v-if="selectType === 'stockList'">
        <!-- 搜索区域 -->
        <a-form layout="inline" @keyup.enter.native="searchQuery">
          <a-row :gutter="24">
            <a-col :md="6" :sm="24">
      ...
      <a-table v-if="selectType === 'list'" bordered ref="table" size="middle" rowKey="id" :columns="columns"
        ...
        <template slot="customRenderStatus" slot-scope="text, record">
          <template v-if="!queryParam.purchaseStatus">
            ...
            <a-tag v-if="record.status === '2' && (queryParam.subType === '采购' || queryParam.subType === '销售')"
              color="green">已审核</a-tag>
            <a-tag v-if="record.status === '3' && (queryParam.subType === '采购' || queryParam.subType === '销售')"
              color="green">已审核</a-tag>
          </template>
          <template v-if="queryParam.purchaseStatus">
      ...
      <a-table v-if="selectType === 'stockList'" bordered ref="table" size="middle" rowKey="id" :columns="stockList"
        :dataSource="dataSource" :pagination="ipagination" :loading="loading"
        :rowSelection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange, type: getType }"
        :customRow="rowAction" @change="handleTableChange">
        <!-- <span slot="numberCustomRender" slot-scope="text, record">
...
export default {
  ...
  methods: {
    ...
    stockShow(type, subType, status) {
      this.selectType = 'stockList'
      this.showType = 'basic'
      this.queryParam.type = type
      this.queryParam.subType = subType
