        ...
        <a-col :md="24">
            <a-card :style="cardStyle" :bordered="false">
                                ...
                                <a-col :md="6" :sm="24">
                                    <a-form-item label="单据状态" :labelCol="labelCol" :wrapperCol="wrapperCol">
                                        <a-select placeholder="选择单据状态" v-model="queryParam.status">
                                            ...
                                            <a-select-option value="2">已转复盘</a-select-option>
                                        </a-select>
                                    </a-form-item>
                                </a-col>
                                <a-col :md="6" :sm="24">
                ...
                <div class="table-operator" style="margin-top: 5px">
                    ...
                    <a-button v-if="checkFlag && btnEnableList.indexOf(2) > -1" icon="check"
                        @click="batchSetStatus(1)">审核</a-button>
                    <a-button v-if="checkFlag && btnEnableList.indexOf(7) > -1" icon="stop"
                        @click="batchSetStatus(0)">反审核</a-button>
                    <a-button v-if="isShowExcel && btnEnableList.indexOf(3) > -1" icon="download"
                    ...
                    <span color="red">注意：库存数据需在盘点复盘后产生</span>
                    <a-tooltip placement="left"
                        title="盘点录入的时候需要填写商品的实存，从而计算出数量，进行保存和审核， 审核之后才能进行盘点复盘。另外可以通过excel批量导入盘点实存信息。" slot="action">
                ...
                <div>
                    <a-table ref="table" size="middle" bordered rowKey="id" :columns="columns" :dataSource="dataSource"
                        ...
                        <span slot="action" slot-scope="text, record">
                            <a @click="myHandleDetail(record, '盘点录入', prefixNo)">查看</a>
                        ...
                        <template slot="customRenderStatus" slot-scope="status">
                            ...
                            <a-tag v-if="status == '2'" color="blue">已转复盘</a-tag>
                        </template>
                    </a-table>
                </div>
                <!-- table区域-end -->
                <!-- 表单区域 -->
                <stock-check-enter-modal ref="modalForm" @ok="modalFormOk"
                    @close="modalFormClose"></stock-check-enter-modal>
...
<!-- by  ji  sheng  hua-->
<script>
import StockCheckEnterModal from './modules/StockCheckEnterModal'
...
import Vue from 'vue'
export default {
    name: "StockCheckEnterList",
    mixins: [JeecgListMixin, BillListMixin],
    components: {
        StockCheckEnterModal,
        BillDetail,
        JEllipsis,
        JDate
    },
