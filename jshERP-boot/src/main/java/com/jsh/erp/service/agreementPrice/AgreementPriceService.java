...
public class AgreementPriceService {
    private Logger logger = LoggerFactory.getLogger(AgreementPriceService.class);

    @Resource
    private AgreementPriceMapper agreementPriceMapper;
    @Resource
    private AgreementPriceMapperEx agreementPriceMapperEx;
    @Resource
    private LogService logService;


    public Long countAgreementPrice(Integer material_id, String batch_number)throws Exception {
        Long result=null;
        try{
            result=agreementPriceMapperEx.countsByAgreementPrice(material_id, batch_number);
        }catch(Exception e){
    ...
    public List<AgreementPrice> select(Integer material_id, String batch_number, int offset, int rows) throws Exception {
        List<AgreementPrice> list = null;
        try {
            list = agreementPriceMapperEx.selectByConditionAgreementPrice(material_id, batch_number, offset, rows);
        } catch (Exception e) {
            JshException.readFail(logger, e);
        }
        return list;
    ...
    public AgreementPrice getAgreementPrice(long id)throws Exception {
        AgreementPrice result=null;
        try{
            result=agreementPriceMapper.selectByPrimaryKey(id);
        }catch(Exception e){
            JshException.readFail(logger, e);
        }
        return result;
    ...
    public List<AgreementPrice> getAgreementPrice()throws Exception {
        AgreementPriceExample example = new AgreementPriceExample();
        example.createCriteria().andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
        List<AgreementPrice> list=null;
        try{
            list=agreementPriceMapper.selectByExample(example);
        }catch(Exception e){
            JshException.readFail(logger, e);
        }
        return list;
    ...
    public int insertAgreementPrice(JSONObject obj, HttpServletRequest request)throws Exception {
        AgreementPrice agreementPrice = JSONObject.parseObject(obj.toJSONString(), AgreementPrice.class);
        int result =0;
        try{
            result=agreementPriceMapper.insertSelective(agreementPrice);
        }catch(Exception e){
            JshException.readFail(logger, e);
        }
        return result;
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public int updateAgreementPrice(JSONObject obj, HttpServletRequest request)throws Exception {
        AgreementPrice AgreementPrice = JSONObject.parseObject(obj.toJSONString(), AgreementPrice.class);
        int result =0;
        try{
            result=agreementPriceMapper.updateByPrimaryKeySelective(AgreementPrice);
        }catch(Exception e){
            JshException.readFail(logger, e);
        }
        return result;
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public int deleteAgreementPrice(Long id, HttpServletRequest request)throws Exception {
        int result =0;
        try{
            result=agreementPriceMapper.deleteByPrimaryKey(id);
        }catch(Exception e){
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public int batchDeleteAgreementPrice(String ids, HttpServletRequest request)throws Exception {
        List<Long> idList = StringUtil.strToLongList(ids);
        AgreementPriceExample example = new AgreementPriceExample();
        example.createCriteria().andIdIn(idList);
        int result =0;
        try{
            result=agreementPriceMapper.deleteByExample(example);
        }catch(Exception e){
            JshException.writeFail(logger, e);
        }
        return result;
    ...
    public int checkIsNameExist(Long id,String batchNumber)throws Exception {
        AgreementPriceExample example = new AgreementPriceExample();
        example.createCriteria().andIdNotEqualTo(id).andBatchNumberNotEqualTo(batchNumber).andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
        List<AgreementPrice> list =null;
        try{
            list=agreementPriceMapper.selectByExample(example);
        }catch(Exception e){
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public int updateIsDefault(Long agreementPriceId) throws Exception {
        int result = 0;
        try {
            //全部取消默认
            AgreementPrice allagreementPrice = new AgreementPrice();
            allagreementPrice.setIsDefault(false);
            AgreementPriceExample agreementPriceExample = new AgreementPriceExample();
            agreementPriceExample.createCriteria();
            agreementPriceMapper.updateByExampleSelective(allagreementPrice, agreementPriceExample);
            //给指定采购协议价为默认
            AgreementPrice agreementPrice = new AgreementPrice();
            agreementPrice.setIsDefault(true);
            AgreementPriceExample example = new AgreementPriceExample();
            example.createCriteria().andIdEqualTo(agreementPriceId);
            agreementPriceMapper.updateByExampleSelective(agreementPrice, example);
            logService.insertLog("采购协议价", BusinessConstants.LOG_OPERATION_TYPE_EDIT + agreementPriceId,
    ...
    public int batchSetStatus(Boolean status, String ids) throws Exception {
        logService.insertLog("采购协议价",
                new StringBuffer(BusinessConstants.LOG_OPERATION_TYPE_ENABLED).toString(),
                ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest());
        List<Long> agreementPriceIds = StringUtil.strToLongList(ids);
        AgreementPrice agreementPrice = new AgreementPrice();
        agreementPrice.setEnabled(status);
        AgreementPriceExample example = new AgreementPriceExample();
        example.createCriteria().andIdIn(agreementPriceIds);
        int result = 0;
        try {
            result = agreementPriceMapper.updateByExampleSelective(agreementPrice, example);
        } catch (Exception e) {
            JshException.writeFail(logger, e);
        }
        return result;
    ...
    public JSONObject parseMapByExcelData(String barCodes, List<Map<String, String>> detailList, String prefixNo) throws Exception {
        ...
        List<MaterialVo4Unit> list = agreementPriceMapperEx.getBillItemByParam(barCodes);
