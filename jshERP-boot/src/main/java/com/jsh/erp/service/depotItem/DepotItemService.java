...
public class DepotItemService {
    ...
    public void saveDetials(String rows, Long headerId, String actionType, HttpServletRequest request) throws Exception{
        ...
        if (null != rowArr && rowArr.size()>0) {
            ...
            for (int i = 0; i < rowArr.size(); i++) {
                ...
                // 记录当前库存
                BigDecimal NowStock = BigDecimal.ZERO;
                BigDecimal AnotherNowStock = BigDecimal.ZERO;
                if (StringUtil.isExist(rowObj.getLong("depotId"))) {
                    depotItem.setDepotId(rowObj.getLong("depotId"));
                    NowStock = getStockByParam(depotItem.getDepotId(), depotItem.getMaterialId(), null, null);
                    depotItem.setCurrentNumber(NowStock); //记录当前库存
                }
                if(BusinessConstants.SUB_TYPE_TRANSFER.equals(depotHead.getSubType())) {
                    if (StringUtil.isExist(rowObj.get("anotherDepotId"))) {
                        depotItem.setAnotherDepotId(rowObj.getLong("anotherDepotId"));
                        AnotherNowStock = getStockByParam(depotItem.getAnotherDepotId(), depotItem.getMaterialId(), null, null);
                        depotItem.setAnotherNumber(AnotherNowStock); //记录当前库存
                    } else {
                ...
                if (StringUtil.isExist(rowObj.get("mealId"))) {
                    depotItem.setMealId(rowObj.getLong("mealId"));
                }
                if (StringUtil.isExist(rowObj.get("depotId"))) {
                    depotItem.setDepotId(rowObj.getLong("depotId"));
                } else {
    ...
    public BigDecimal getCurrentStockByAllDepot(List<Long> allList, Long mId) {
        BigDecimal stock = depotItemMapperEx.getCurrentStockByAllDepot(allList, mId);
        return stock!=null? stock: BigDecimal.ZERO;
    }
