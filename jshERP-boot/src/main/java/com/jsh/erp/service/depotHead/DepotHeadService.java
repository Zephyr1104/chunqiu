...
import static com.jsh.erp.utils.Tools.*;
        ...
        try {
            ...
            if (null != list) {
                ...
                for (DepotHeadVo4List dh : list) {
                    ...
                    // 添加调入库存名字
                    List<DepotItem> listByHeaderId = depotItemService.getListByHeaderId(dh.getId());
                    if (null != listByHeaderId) {
                        List<Long> depotIds = new ArrayList<>();
                        List<Long> AnotherDepotIds = new ArrayList<>();
                        for (DepotItem depotItem : listByHeaderId) {
                            if (!depotIds.contains(depotItem.getDepotId())) {
                                depotIds.add(depotItem.getDepotId());
                            }
                            if (!AnotherDepotIds.contains(depotItem.getAnotherDepotId())) {
                                AnotherDepotIds.add(depotItem.getAnotherDepotId());
                            }
                        }
                        String depotIdsString = StringUtils.join(depotIds, ",");
                        String AnotherdepotIdsString = StringUtils.join(AnotherDepotIds, ",");

                        String setDepotName = "";
                        String setAnotherDepotName = "";

                        if (listByHeaderId.get(0).getDepotId() != null) {
                            List<Depot> depotList = depotService.getDepotListByIds(depotIdsString);
                            for (Depot depot : depotList) {
                                setDepotName += depot.getName() + ",";
                            }
                        }
                        if (listByHeaderId.get(0).getAnotherDepotId() != null) {
                            List<Depot> AnotherDepotList = depotService.getDepotListByIds(AnotherdepotIdsString);
                            for (Depot depot : AnotherDepotList) {
                                setAnotherDepotName += depot.getName() + ",";
                            }
                        }

                        dh.setDepotName(setDepotName);
                        dh.setAnotherDepotName(setAnotherDepotName);
                    }
...
//                        dh.setTotalPrice(roleService.parseBillPriceByLimit(dh.getTotalPrice().abs(), billCategory, priceLimit, request));
                        dh.setTotalPrice(roleService.parseBillPriceByLimit(dh.getTotalPrice(), billCategory, priceLimit, request));
                    }
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public int batchSetStatus(String status, String depotHeadIDs, String auditor) throws Exception {
        int result = 0;
        ...
        if (dhIds.size() > 0) {
            ...
            // 记录当前审核的人
            depotHead.setAuditor(Long.valueOf(auditor));
    ...
    public List<DepotHeadVo4List> getDetailByNumber(String number, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            if (null != list) {
                ...
                if (dh.getOperTime() != null) {
                    dh.setOperTimeStr(parseDateToStr(dh.getOperTime()));
                }
                //商品信息简述
                if (materialsListMap != null) {
                    dh.setMaterialsList(materialsListMap.get(dh.getId()));
                ...
                // 添加审核员名字
                if (dh.getAuditor() != null) {
                    dh.setAuditorName(userService.getUser(dh.getAuditor()).getUsername());
                } else {
                    dh.setAuditorName("暂无审核人");
                }
                resList.add(dh);
            }
        } catch (Exception e) {
    ...
    public void debtExport(Long organId, String materialParam, String number, String type, String subType,
        ...
        try {
            ...
            File file = new File("/opt/opt" + fileName);
