...
public class DepotItemController {
    ...
    public String findDetailByDepotIdsAndMaterialId(
        ...
        if (list != null) {
            for (DepotItemVo4DetailByTypeAndMId d: list) {
                ...
                item.put("currentNumber",d.getCnum()); // 调出仓-当前库存
                item.put("anotherNumber",d.getAnum()); // 调入仓-当前库存
        ...
        try {
            ...
            if (null != dataList) {
                ...
                for (DepotItemVo4WithInfoEx diEx : dataList) {
                    ...
                    item.put("mealId", diEx.getMealId() == null ? "" : diEx.getMealId());
                    item.put("mealName", diEx.getMealId() == null ? "" : diEx.getMealName());
     ...
     * @return
     */
    @PostMapping(value = "/importStockCheckExcel")
    public BaseResponseInfo importStockCheckExcel(MultipartFile file,
                                                  @RequestParam(required = false, value = "prefixNo") String prefixNo,
                                                  HttpServletRequest request, HttpServletResponse response) throws Exception{
        BaseResponseInfo res = new BaseResponseInfo();
        Map<String, Object> data = new HashMap<>();
        ...
        try {
            ...
            if(src.getRows()>1000) {
                ...
                for (int i = 2; i < src.getRows(); i++) {
                    ...
                    String currentStock = ExcelUtils.getContent(src, i, 2);
                    Map<String, String> materialMap = new HashMap<>();
                    materialMap.put("barCode", barCode);
                    materialMap.put("currentStock", currentStock);
                    detailList.add(materialMap);
                    barCodes += "'" + barCode + "',";
                }
                if (StringUtil.isNotEmpty(barCodes)) {
     ...
     * @return
     */
    @PostMapping(value = "/importTransferExcel")
    public BaseResponseInfo importTransferExcel(MultipartFile file,
                                                  @RequestParam(required = false, value = "prefixNo") String prefixNo,
                                                  HttpServletRequest request, HttpServletResponse response) throws Exception{
        BaseResponseInfo res = new BaseResponseInfo();
        Map<String, Object> data = new HashMap<>();
        ...
        try {
            ...
            if(src.getRows()>1000) {
                ...
                for (int i = 2; i < src.getRows(); i++) {
                    String barCode = ExcelUtils.getContent(src, i, 0);
                    String num = ExcelUtils.getContent(src, i, 4);
                    String unitPrice = ExcelUtils.getContent(src, i, 5);
                    String outDepotName = ExcelUtils.getContent(src, i, 8);
                    String inDepotName = ExcelUtils.getContent(src, i, 9);

                    Map<String, String> materialMap = new HashMap<>();
                    materialMap.put("barCode", barCode);
                    materialMap.put("num", num);
                    materialMap.put("unitPrice", unitPrice);
                    materialMap.put("depotName",outDepotName);
                    materialMap.put("anotherDepotName",inDepotName);
                    detailList.add(materialMap);
                    barCodes += "'" + barCode + "',";
                }
                if (StringUtil.isNotEmpty(barCodes)) {
