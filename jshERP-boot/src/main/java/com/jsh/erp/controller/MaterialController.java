...
public class MaterialController {
    ...
    public String batchSetStatus(@RequestBody JSONObject jsonObject,
        ...
        // 判断该商品是否有库存,如果有库存则不能禁用
        if(status == false) {
            List<Depot> allList = depotService.getAllList();
            List<Long> depotIds = new ArrayList<>();
            for(Depot d: allList) {
                depotIds.add(d.getId());
            }
            List<Long> mIds = StringUtil.strToLongList(ids);
            BigDecimal stock = BigDecimal.ZERO;
            for (Long mId : mIds) {
                stock = depotItemService.getCurrentStockByAllDepot(depotIds, mId);
                if (stock != null && stock.compareTo(BigDecimal.ZERO) > 0){
                    Map<String, Object> map = new HashMap<>();
                    return returnJson(map, ErpInfo.DISABLE_MSG.name, ErpInfo.DISABLE_MSG.code);
                }
            }
        }
